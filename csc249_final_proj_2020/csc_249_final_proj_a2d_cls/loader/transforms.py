...
def rescale(img, scales, interpolation=cv2.INTER_LINEAR):
    if isinstance(scales, list):
        if scales[0] < 100:
            ...
            is_factor = False
            if len(scales) == 2:
                target_size = (scales[0], scales[1])
            else:
                target_size = (scales[0], scales[0])
    else:
        if scales < 100:
            is_factor = True
            scale = scales
            ...
            is_factor = False
            target_size = (scales, scales)
    if is_factor:
        img = cv2.resize(
            img, None, fx=scale, fy=scale, interpolation=interpolation)
    else:
        img = cv2.resize(
            img, target_size, interpolation=interpolation)
